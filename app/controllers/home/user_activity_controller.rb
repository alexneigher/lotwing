class Home::UserActivityController < ApplicationController
  ...
  def index
    ...
    grouped_by_user_events.each do |user_id, events|
      last_day_events = events.select{|e| e.created_at >= 1.day.ago.beginning_of_day}.count
      last_week_events = events.select{ |e| e.created_at >= 1.week.ago.beginning_of_day}.count
      mtd_events = events.select{|e| e.created_at >= Date.today.beginning_of_month}.count
